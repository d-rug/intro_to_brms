...
Another model function is *brm_multiple* for when you have multiple datasets that you would like to run your model over. Specifically, this is helpful if you missing data and your dataset would benefit from multiple imputation. `brms` interfaces nicely with the `mice` package which allows you to pass multiple imputed datasets into your model which then combines the posteriors draws into one model result. You can then run the rest of the `brms` post-processing methods as you typically would.
